            ...
            <?php if ($condition): ?>
                <tr>
                    ...
                    <?php if ($pageName === "index"): ?>
                        <td><a href="index.php?action=add&id=<?= $product['id'] ?>"
                               class="btn btn-primary"><?= translate('add') ?></a></td>
                    <?php endif ?>

                    <?php if ($pageName === "products"): ?>
                        <td><a href="product.php?action=edit&id=<?= $product['id'] ?>"
                               class="btn btn-info"><?= translate('edit') ?></a></td>
                        <td><a href="product.php?action=delete&id=<?= $product['id'] ?>&image=<?= $product['image'] ?>"
                               class="btn btn-danger"><?= translate('delete') ?></a></td>
                    <?php endif ?>

                    <?php if ($pageName === "cart"): ?>
                        <td><a href="cart.php?action=remove&id=<?= $product['id'] ?>"
                               class="btn btn-danger"><?= translate('remove') ?></a></td>
                    <?php endif ?>
                </tr>
            <?php endif ?>
        <?php endforeach ?>
